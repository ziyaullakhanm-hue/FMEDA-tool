-- Single-file SQL dump for SafeCrate example DB
-- This file includes minimal DDL needed for types/subtypes/variants
-- and inserts the example `component_variants` rows used by the demo.

BEGIN;

-- Ensure extension for UUID generation (Postgres)
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Minimal tables required for variants (migrations also create full schema)
...
CREATE TABLE IF NOT EXISTS component_variants (
  ...
  name TEXT NOT NULL,
  ref_fit DOUBLE PRECISION NOT NULL,
  ref_temp DOUBLE PRECISION NOT NULL,
  a DOUBLE PRECISION,
  ea1 DOUBLE PRECISION,
  ea2 DOUBLE PRECISION,
  c2 DOUBLE PRECISION,
  c3 DOUBLE PRECISION,
  uref_umax_ratio DOUBLE PRECISION,
  pi_q DOUBLE PRECISION,
  notes TEXT,
  created_at TIMESTAMP DEFAULT NOW(),
...
VALUES ('Resistor', 'Passive resistor components')
ON CONFLICT (name) DO NOTHING;

INSERT INTO component_types (name, description)
VALUES ('Capacitor', 'Passive capacitor components')
ON CONFLICT (name) DO NOTHING;

-- Insert subtypes
INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Aluminium electrolytic', 'Aluminium electrolytic capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;

INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Ceramic', 'Ceramic capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;

INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Glass', 'Glass/dielectric capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;

INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Metal foil', 'Metal foil capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;

INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Mica', 'Mica capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;

INSERT INTO component_subtypes (type_id, name, description)
SELECT id, 'Tantalum electrolytic', 'Tantalum electrolytic capacitors' FROM component_types WHERE name = 'Capacitor'
ON CONFLICT DO NOTHING;
...
-- Insert many variants (IDs and values from provided dataset)
...
ON CONFLICT (subtype_id, name) DO NOTHING;
...
-- Additional condensed variants (some thermal constants unknown)
INSERT INTO component_variants (id, subtype_id, name, ref_fit, ref_temp, c2, c3, uref_umax_ratio, pi_q, a, ea1, ea2, notes, created_at)
VALUES
...
-- Resistors (Thin/Thick film examples)
